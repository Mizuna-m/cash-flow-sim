...
* forecast の根拠をさらに説明できる粒度まで API を広げる
* simulation の危険日説明をイベント単位まで掘れるようにする
* シナリオ比較 API と ON/OFF 切替の土台を入れる
...
| M4 | シミュレーションコア実装 | DOING | 日次残高・カード残高・ショート判定が計算できる | DB seed を使う simulation API まで実装済み。日別 event summary 追加済み。イベント単位 explanation は未実装 |
...
1. simulation の危険日 explanation をイベント単位で返す
2. シナリオ比較の土台となる ON/OFF 設計を入れる
3. 比較 UI を mock から API 接続へ置き換える
4. 初回移行導線の入力補助を作る
