...
加えて、本システムの UI / UX は「判断そのものを代行すること」よりも、
まず fact を誤解なく並べて見せることを優先する。

ここでいう fact とは、例えば次を指す。

* 日次の `cash`
* 口座別の `cash_by_account`
* カード別の `card_debt`
* 将来支出として残っている `planned_outflow`
* その日付にどのイベントが `planned / confirmed / settled` なのか

ユーザーが「何をすべきか」を最終判断する主体であり、short 判定や compare 差分は補助情報として扱う。
...
* fact を元に計算される重要な観測値
* ユーザー判断に直結する予測値として扱う
* 元データとして永続化はしないが、UI 上は補助アラートより優先して読めるべき値とする
...
ただし、UI の主目的は「払える / 払えない」を自動で断定することではなく、
判断に必要な fact を日次で確認できるようにすることである。

そのため、表示の優先順位は次の順とする。

1. 日次の金額事実
2. 口座別の金額事実
3. `projected_cash` のような fact 由来の計算済み観測値
4. イベントの lifecycle と内訳
5. 補助的な判定や比較指標
...
ただし short はプロダクトの中心成果物ではない。

位置づけ:

* short は資金繰り確認の補助アラート
* ユーザーの判断を置き換えるものではない
* UI 上では、日次残高・口座別残高・`projected_cash`・イベント内訳の後に補助的に提示する
...
* `projected_cash` は補助アラートではなく、fact 由来の重要な計算済み観測値として扱う
* 将来見込みの説明は `planned_outflow` と `projected_cash` の関係で表現する
